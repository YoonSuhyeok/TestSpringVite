...
// ../../node_modules/.pnpm/tsup@8.1.0_@swc+core@1.5.27_postcss@8.4.38_typescript@5.4.5/node_modules/tsup/assets/esm_shims.js
...
var filter2 = process2.env.VITE_DEBUG_FILTER;
...
function createRollupInputConfig(pattern = "src/**/main.ts", baseDir = "src", options = {}) {
  ...
  return Object.fromEntries(
    globSync(pattern, globOptions).map((file) => {
      ...
      return [
        path2.relative(baseDir, filepath.slice(0, filepath.length - path2.extname(filepath).length)),
        absolutePath
      ];
    })
  );
...
function dirname() {
  return fileURLToPath3(new URL(".", import.meta.url));
}
function isMavenProject(projectRoot = "") {
  return fs.existsSync(path2.join(projectRoot || process2.cwd(), "pom.xml"));
}
function isGradleProject(projectRoot = "") {
  return fs.existsSync(path2.join(projectRoot || process2.cwd(), "build.gradle"));
}
function isKotlinDSLProject(projectRoot = "") {
  return fs.existsSync(path2.join(projectRoot || process2.cwd(), "build.gradle.kts"));
}
function readPropertiesFile(pattern, options) {
  const properties = /* @__PURE__ */ new Map();
  const _pattern = pattern || "**/*.properties";
  const _options = merge({}, options || {}, { cwd: process2.cwd() });
...
function javaVersion(projectRoot = ".") {
  try {
    ...
    if (isMavenProject(projectRoot)) {
      const pom = fs.readFileSync(path2.join(projectRoot, "pom.xml")).toString();
      version = pom.match(/<java.version>(.*)<\/java.version>/)?.[1];
    }
    if (isGradleProject(projectRoot)) {
      const gradle = fs.readFileSync(path2.join(projectRoot, "build.gradle")).toString();
    ...
    if (isKotlinDSLProject(projectRoot)) {
      const gradleKts = fs.readFileSync(path2.join(projectRoot, "build.gradle.kts")).toString();
...
function pluginVersion() {
  try {
    return JSON.parse(fs.readFileSync(path2.join(dirname(), "../package.json")).toString())?.version;
  } catch {
    return "";
  }
}
...
function resolveJavaPlugin(pluginConfig) {
  ...
  const plugins = [
    {
      ...
      config: (config, { command, mode }) => {
        ...
        userConfig = config;
        const env = loadEnv(mode, config.envDir || process3.cwd(), "");
      ...
      configureServer(server) {
        const envDir = resolvedConfig.envDir || process3.cwd();
  ...
  ${import_picocolors.default.red(`${import_picocolors.default.bold("JAVA")} ${javaVersion(pluginConfig.javaProjectBase)}`)}  ${import_picocolors.default.dim("plugin")} ${import_picocolors.default.bold(`v${pluginVersion()}`)}`);
        ...
        if (!exitHandlersBound) {
          process3.on("SIGINT", () => process3.exit());
          process3.on("SIGTERM", () => process3.exit());
          process3.on("SIGHUP", () => process3.exit());
        ...
        return () => server.middlewares.use((req, res, next) => {
          if (req.url === "/index.html") {
            res.statusCode = 404;
            res.end(
              fs2.readFileSync(path3.join(dirname(), "dev-server-index.html")).toString().replace(/\{\{ APP_URL \}\}/g, appUrl)
            );
          }
          next();
        });
...
function resolveEnvironmentServerConfig(env) {
  ...
  if (!fs2.existsSync(env.VITE_DEV_SERVER_KEY) || !fs2.existsSync(env.VITE_DEV_SERVER_CERT)) {
  ...
  return {
    ...
    host,
    https: {
      key: fs2.readFileSync(env.VITE_DEV_SERVER_KEY),
      cert: fs2.readFileSync(env.VITE_DEV_SERVER_CERT)
    }
  };
}
